...
async def test_delete_category_with_child(
    ...
    child_category: Category = await Category.find_one({"parent_name": {"$ne": None}})
...
async def test_delete_category_with_assigned_part(
    client: AsyncClient, categories: InsertManyResult, parts: InsertManyResult
):
    # Arrange
...
async def test_create_part_with_base_category(
    ...
    expected_response: str = f'Could not assign part to base category "{category.name}"'
    # Act
    response: Response = await client.post("/parts", content=json.dumps(part_data))
    # Assert
    assert response.status_code == status.HTTP_409_CONFLICT
...
async def test_update_part_category_not_exists(
    client: AsyncClient, parts: InsertManyResult, categories: InsertManyResult
):
    # Arrange
